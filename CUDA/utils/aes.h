#pragma once

#include <inttypes.h>
#include <string.h>
#include <stdlib.h>
#include <stdio.h>


//sbox on static memory
const unsigned char sbox[256] ={
	
};

//round constants
const unsigned char rc[10] ={
	0x01, 0x02, 0x04, 0x08, 0x10,
	0x20, 0x40, 0x80, 0x1b, 0x36
};

#define get_sbox_value(num) (sbox[(num)])



void build_subkeys(unsigned char* key,unsigned char* sub_keys, int rowsize,int rounds);

// dev functions

__device__ void byte_sub(unsigned char *buff,unsigned char* sbox);
__device__ void shift_rows(unsigned char *buff);
__device__ void mix_columns(unsigned char *buff);
__device__ void xor_key(unsigned char *buff, unsigned char *key, const unsigned int &round);

// kernel function
__global__ void aes_encript(unsigned char *mat, unsigned char *result, unsigned char *sbox, unsigned char *keys, unsigned int width);
